...
function Loot({ it, index }) {
  ...
  return (
    <div className="fb-loot" style={delay}>
      ...
      <div className="mx-auto w-28 h-28 rounded-2xl bg-white/10 p-3 border-2" style={{ borderColor: tier.glow }}>
...
export function ChestModal({ chest, remaining = 0, onOpen, onNext, onShowBox, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/90 grid place-items-center px-6 overflow-y-auto py-8">
      {phase === "open" && <Confetti />}
      <div className="w-full max-w-xs text-center">
        ...
        <button type="button" onClick={open} aria-label="Ouvrir le coffre"
          className={`block mx-auto transition-[margin] duration-300 ${phase === "open" ? "mt-12" : "mt-3"} ${phase === "closed" ? "fb-chest" : phase === "shaking" ? "fb-chest-hard" : ""}`}>
          <ChestArt tier={chest.tier} open={phase === "open"} size={phase === "open" ? 150 : 190} />
        </button>
        ...
        {phase === "open" && result && (
          <div className="mt-2 space-y-3">
            ...
            <div className="pt-3 space-y-2 fb-slide-in" style={{ animationDelay: ".6s" }}>
              {hasCollectible && (
        ...
        {phase === "closed" && (
          <button onClick={onClose} className="mt-6 text-xs font-bold text-white/40">Plus tard</button>
        )}
      </div>
    </div>
  );
